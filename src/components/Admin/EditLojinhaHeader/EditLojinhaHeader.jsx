...
const EditLojinhaHeader = () => {
  ...
  useEffect(() => {
    const fetchLojinhaData = async () => {
      try {
        // Caminho fixo para o documento de dados de configuração da lojinha
        const lojaRef = doc(db, "config", "lojinhaHeader"); // Caminho correto
  ...
  const handleLogoUpload = async (file) => {
    ...
    try {
      ...
      setLogoUrl(response.data.secure_url); // Salva a URL retornada do Cloudinary
    } catch (error) {
